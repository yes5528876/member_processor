...
def login_process():
    ...
    try:
        ...
        capabilities = driver.capabilities  # This should work
...
def search_one():
    ...
    if df['單位'][index] != unit:
        ...
        new_row = pd.DataFrame({"事件": [df['姓名'][index] + "  調轉單位到  " + unit]})
        log_DataFrame = pd.concat([log_DataFrame, new_row], ignore_index=True)
        row['單位'] = unit
